...
async function handleErrors(res: Response) {
  if (!res.ok) {
    ...
    const errorRes: ErrorResponse = {message: error.message, status: res.status, url: res.url}
    throw errorRes;
  }
  return res;
}
...
async function httpGetUserRepo(repo_url: string, setError: (error: ErrorResponse) => void) {
  try {
...
async function httpGetOrganisationUrl(organizations_url: string, setError: (error: ErrorResponse) => void) {
  try {
...
async function httpGetUserByLogin(login: string, setError: (error: ErrorResponse) => void) {
  try {
...
async function httpGetFollowerUrl(following_url: string, setError: (error: ErrorResponse) => void) {
  try {
